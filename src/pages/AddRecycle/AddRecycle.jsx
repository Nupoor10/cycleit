...
const AddRecycle = () => {
  ...
  const addItem = async() => {
    try {
      if(user) {
        ...
        const response = await axios.post(`${apiURL}/recycle/add/`, { object: detection }, config);
  ...
  return (
    <div className="scan__recycle__page">
      ...
      {detection && (
        <div className="detection__placeholder">
          ...
          {!isDetecting && (
            <button onClick={addItem} className="primary__btn">
              ADD
            </button>
          )}
        </div>
